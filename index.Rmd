...
Excluidos <- Edades %>%
  filter(Prénom %in% c('NOÉMIE', 'CEDRIC', 'GREGORY', "ALEXIS", "CINDY", "STÉPHANE", "PEDRO", "JONATHAN", "CHRISTIAN", "DANIEL ANDRES", "MARIE REINE")) 

Edades <- Edades %>% anti_join(Excluidos)

library(lemon)

piramide <- Edades %>% group_by(rango, Sexe) %>% tally()
```
...
#mapCountryData(spdf, mapTitle="Collaboration ERPI", nameColumnToPlot="Collaborations", catMethod=c(1:100))
...
df$count_group <- cut(df$count, 
                      ...
                      labels = c("Iniciant", "En construction", "Recurrent" ))
