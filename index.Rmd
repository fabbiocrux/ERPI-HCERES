...
source(here("R/IF.R"))
IF.evolution
#ggsave(filename="Figures/IF.evolution-August-2022.png", plot = IF.evolution, width = 9, height = 5, dpi="print" )
...
Articles %>% 
   group_by(Year, Scopus) %>% filter(Year > 2011) %>% 
   tally(name = "Quantity") %>%
...
ggsave(filename="Figures/Scopus-perfil.jpg", width = 9, height = 5, dpi="print" )
...
Scimago
#ggsave(filename="Figures/Scimago.jpg", plot = Scimago, width = 9, height = 5, dpi="print" )
...
status %>% 
   ...
   labs(y="Nombre des personnes", 
        ...
        subtitle = paste0("Effectif total du personnel à la date de 31/12/2021: ", nrow(status)),
        caption =  paste0("Denière mise à jour: ", format(Sys.time(), '%d/%m/%Y'))) +
   theme_fabio() +
...
#ggsave(filename="Figures/Edades.jpg",  width = 9, height = 5, dpi="print" )
#?write_csv2(x=Members, file = "ERPI/Edades.csv" )
