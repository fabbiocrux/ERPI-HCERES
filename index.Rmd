...
# Impact Factor ----
IF <- Revistas  %>% drop_na(Impact_factor_2019) %>% group_by(Journal) %>% summarise(Quantity=n()) 
IF <- IF %>% 
        left_join(Revistas %>% select(Journal,Impact_factor_2019) %>% unique(), by="Journal") %>% 
...
IF %>%
   ...
   geom_point(size=3, color="#69b3a2") +
  geom_text( label=paste0("(", IF$Quantity, ")"), nudge_x = 0, nudge_y = 0.25, check_overlap = T, size=3, color="#69b3a2" )+
   coord_flip() +
   theme_fabio() +
   #theme_minimal(base_size = 15, base_family = "Palatino") +
   labs(x = "", y = "Impact Factor \n( Quantité d'articles publies)", title = "Journaux scientifiques principaux",
        subtitle = "") +
   #    theme_ipsum() +
   theme(
      panel.grid.minor.y = element_blank(),
